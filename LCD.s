...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		...
		PUSH {R4, R5, R6, LR}			; push register states
	
cmdBUSY
	LDR R4, =SSI0_SR_R				; check status bit
	LDR R5, [R4]			
    ANDS R5, #0x10					; isolate bit 4
	BNE cmdBUSY						; if HIGH, wait
	
	LDR R4, =DC
	LDR R5, =SSI0_DR_R
	LDRB R6, [R4]
	AND R6, #0						; clear DC
	STRB R6, [R4]					; set PA6 0
	
	STRB R0, [R5]					; write command to SSI0_DR_R
	
cmdBUSY1
	LDR R4, =SSI0_SR_R				; check status bit
	LDR R5, [R4]
	ANDS R5, #0x10
	BNE cmdBUSY1					; if HIGH, wait
...
dataBusy
	LDR R4, =SSI0_SR_R
	LDRB R5, [R4]
	ANDS R5, #0x1					; isolate bit 1
	BEQ dataBusy					; if [1] = 0, wait
	
	LDR R4, =DC
	MOV R5, #0x40
	STRB R5, [R4]					; move 1 into DC
	
	LDR R4, =SSI0_DR_R
	STRB R0, [R4]					; move data into DATA_R
